...
let yfSession = null; // { crumb, cookie }
...
async function getYFSession() {
    ...
    console.log('YF session established. Crumb:', crumb.slice(0, 8) + '...');
    return yfSession;
}
...
app.get('/api/sentiment/:ticker', async (req, res) => {
    ...
    try {
        ...
        // Return last 8 tagged messages for display
        ...
        console.error(`[sent:${ticker}]`, err.message);
        res.status(500).json({ ok: false, ticker, error: err.message });
    }
});
...
app.get('/api/news/:ticker', async (req, res) => {
    ...
    try {
        ...
        const news = raw.map(n => ({
            ...
            link:      n.link,
            time:      n.providerPublishTime,   // unix seconds
...
app.get('/api/tv/:ticker', async (req, res) => {
    ...
    try {
        ...
        console.error(`[tv:${ticker}]`, err.message);
        res.status(500).json({ ok: false, ticker, error: err.message });
    }
});
...
app.get('/api/scanner/:profile', async (req, res) => {
    ...
    if (!profile) return res.status(400).json({ ok: false, error: 'Unknown profile. Valid: ' + Object.keys(SCAN_PROFILES).join(', ') });
    ...
    try {
        ...
        console.error(`[scanner:${req.params.profile}]`, err.message);
...
app.get('/api/political-news/:topic', async (req, res) => {
    ...
    try {
        ...
        console.error(`[polnews:${topic}]`, err.message);
        res.status(500).json({ ok: false, error: err.message });
    }
});
...
// ──────────────────────────────────────────────
// SNAPSHOTS (auto-save scores for history)
// ──────────────────────────────────────────────
app.post('/api/snapshots', async (req, res) => {
    const { snapshots } = req.body; // [{ ticker, price, score, rsi, tv_signal, upside }]
...
if (process.env.VERCEL !== '1') {
    app.listen(PORT, () => {
        ...
        getYFSession().catch(e => console.warn('Session warm-up failed:', e.message));
    });
}
